...
export function CategoryForm({ category, onSaved }: CategoryFormProps) {
  ...
  const [parentCategoryId, setParentCategoryId] = React.useState(category?.parentCategoryId || "");
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!category) {
        saved = await createCategory(name.trim(), parentCategoryId || null);
      } else {
        saved = await updateCategory(category.id, name.trim(), parentCategoryId || null);
      }
      onSaved?.(saved);
      if (!category) { setName(""); setParentCategoryId(""); }
    } catch (err: any) {
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-3">
      ...
      <div>
        <input value={parentCategoryId} onChange={e => setParentCategoryId(e.target.value)} placeholder="Parent Category Id (optional)" className="w-full rounded border px-3 py-2 text-sm" />
      </div>
